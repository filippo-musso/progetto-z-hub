...
import { Plus, Trash2, Play, Loader2, Copy, Download, FileText } from "lucide-react";
...
function NewInvoicingForm() {
  ...
  const mutation = useMutation({
    mutationFn: startInvoicing,
    onSuccess: () => {
      toast.success("Fatturazione avviata");
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!confirm("Vuoi avviare la fatturazione con i parametri indicati?")) return;
    mutation.mutate({
      dateFrom,
      dateTo,
...
function JobCard({ job }: { job: Awaited<ReturnType<typeof listInvoicingJobs>>[number] }) {
  ...
  const statusBadge = {
    ...
    completed: <Badge className="bg-green-500/15 text-green-700 dark:text-green-300 border-green-500/30">Completata</Badge>,
        ...
        {open && job.log && (
          <div className="mt-4 space-y-2">
            ...
            <ScrollArea className="h-64 rounded-md border bg-muted/30 p-3">
              <pre className="text-xs font-mono whitespace-pre-wrap">{job.log}</pre>
            </ScrollArea>
          </div>
        )}
      </CardContent>
